...
listuser = connection.execute("select name from sqlite_master where type='table' AND name='user'").fetchall()
...
@user.route("/complaintreport",methods=["POST","GET"])
def complaint_report():
    if request.method=="POST":
        ...
        try:
            connection.execute("insert into user(date,remark,description)values('"+getdate+"','"+getremark+"','"+getdescription+"')")
            connection.commit()
            print("Inserted Successfully")
        except Exception as e:
            print(e)

    return render_template("complaintreport.html")
